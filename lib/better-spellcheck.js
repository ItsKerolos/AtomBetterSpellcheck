...
function init()
{
  ...
  // load the en_US dictionary affix
  readFile(join(__dirname, '../dictionaries/index.aff'), (err, aff) =>
  {
    ...
    // load the en_US & cSpell dictionary
...
function spellcheck(editor)
{
  return new Promise((resolve, reject) =>
  {
    ...
    // export all words from the editor based on a regex
    const words = editor.getText().match(regex);
    ...
    for (let i = 0; i < words.length; i++)
    {
      ...
      // if misspelled then get corrections
      if (!spell.correct(word))
        misspelled[word] = [];
        // misspelled[word] = spell.suggest(word);
    }

    console.log(JSON.stringify(misspelled));
  });
}
