...
*
* Group (1) (Starts with symbols)
* ([a-zA-Z]+'[a-zA-Z]+)
* match words that stars with selected symbols
* [isn't]
*
* Group (2) (Starts with symbols)
* ((.'.)?.+)
* match words that have an apostrophe in them
* [isn't]
*
* Group (3) (camelCase)
* ([A-Z][a-z]+)
...
* [ALL]_[CAPS]
*/
const wordRegex = /(#[^\s]*)|([a-zA-Z]+'[a-zA-Z]+)|([A-Z][a-z]+)|([a-z]+)|([A-Z]+(?=[A-Z][a-z]+))|([A-Z]+)/g;
...
{
  for (const editorPath in activeEditors)
  {
    const activeEditor = activeEditors[editorPath];

    activeEditor.markerLayer.clear();

    activeEditor.gutter.destroy();

    activeEditors[editorPath] = undefined;
  }
...
function init()
{
  ...
  delayCheck = atom.config.get('better-spellcheck.delayCheck');
  characterLimit = atom.config.get('better-spellcheck.characterLimit');

  // observe configuration
  atom.config.observe('better-spellcheck.delayCheck', (value) => delayCheck = value);
  ...
  readFile(join(__dirname, '../dictionaries/index.aff'), (err, aff) =>
  {
    ...
    readFile(join(__dirname, '../dictionaries/cspell.dic'), (err, dic) =>
    {
      ...
      buildDictionary(affBuffer, dicBuffer, () =>
...
* @param { () => void } callback
*/
function buildDictionary(aff, dic, callback)
{
...
function observeEditor(editor)
{
  ...
  const gutter = editor.addGutter({ type: 'decorated', priority: -1 });
  ...
  disposable.add(editor.onDidChange(() =>
  {
    ...
    // delay the spellcheck for ...ms to make sure the user ain't still typing
    spellcheckTimeout = setTimeout(() => spellcheck(editor), delayCheck);
  }));
...
function spellcheck(editor)
{
  return new Promise((resolve) =>
  {
    ...
    {
      const word = result.matchText.trim();

      if (word.length <= 2 || word.match(/(#[^\s]*)/g))
        return;
    ...
    requestAnimationFrame(() =>
    {
      ...
      */
      ...
      addButton(`Add: "${word}" to dictionary`, () =>
      {
        ...
        // build new dictionary then re-trigger a spellcheck
        buildDictionary(affBuffer, dicBuffer, () =>
          spellcheck(editor));
      });
